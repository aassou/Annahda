...
//classes loading begin
    ...
    if(isset($_SESSION['userMerlaTrav']) and $_SESSION['userMerlaTrav']->profil()=="admin"){
		...
		if(isset($_GET['idProjet']) and ($_GET['idProjet'])>0 and $_GET['idProjet']<=$projetManager->getLastId()){
			...
			$livraisonNumber = $livraisonManager->getLivraisonsNumberByIdProjet($idProjet);
			if($livraisonNumber != 0){
		        ...
		        $pagination = paginate('livraisons-list.php?idProjet='.$idProjet, '&p=', $pageNumber, $p);
					...
					<div class="span12">
						<div class="row-fluid add-portfolio">
							...
							<div class="pull-right">
								<a href="livraison-add.php?idProjet=<?= $idProjet ?>" class="btn icn-only green">Ajouter Nouvelle Livraison <i class="icon-plus-sign "></i></a>
							</div>
							<?php
							}
							?>
						...
						<div class="portlet">
							<div class="portlet-title">
								<h4>Liste des livraisons du projet : <strong><?= $projet->nom() ?></strong></h4>
								<div class="tools">
								...
								<table class="table table-striped table-bordered table-advance table-hover">
									...
									<tbody>
										...
										<tr>
											<td>
												<div class="btn-group">
												    ...
												    <ul class="dropdown-menu">
												        <li>
												        	<a href="livraison.php?codeLivraison=<?= $livraison->code() ?>">
												        		Fiche descriptif
												        	</a>
												        	<a href="#addPieces<?= $livraison->id() ?>" data-toggle="modal" data-id="<?= $livraison->id() ?>">
																Ajouter Document
															</a>
															<a href="livraison-pieces.php?idProjet=<?= $idProjet ?>&idLivraison=<?= $livraison->id() ?>" data-toggle="modal" data-id="<?= $livraison->id() ?>">
											...
											<td class="hidden-phone"><?= number_format($livraison->prixUnitaire(), 2, ',', ' ') ?></td>
											<td><a><?= number_format($livraison->prixUnitaire()*$livraison->quantite(), 2, ',', ' ') ?></a></td>
