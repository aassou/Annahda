...
class LivraisonDetailManager{
    ...
    public function add(LivraisonDetail $livraison){
        $query = $this->_db->prepare('INSERT INTO t_livraison_detail (designation, quantite, 
        prixUnitaire, idLivraison)
        VALUES (:designation, :quantite, :prixUnitaire, :idLivraison)') 
    ...
    public function update(LivraisonDetail $livraison){
        $query = $this->_db->prepare('UPDATE t_livraison_detail SET designation=:designation,
        quantite=:quantite, prixUnitaire=:prixUnitaire WHERE id=:id') 
        or die(print_r($this->_db->errorInfo()));
        $query->bindValue(':designation', $livraison->designation());
        $query->bindValue(':quantite', $livraison->quantite());
        $query->bindValue(':prixUnitaire', $livraison->prixUnitaire());
