...
class CaisseIaazaManager{
    ...
    public function add(Caisse $caisse){
        $query = $this->_db->prepare(' INSERT INTO t_caisse_iaaza (
    ...
    public function update(Caisse $caisse){
        $query = $this->_db->prepare(' UPDATE t_caisse_iaaza SET 
    ...
    public function delete($id){
        $query = $this->_db->prepare(' DELETE FROM t_caisse_iaaza
        WHERE id=:id')
        or die (print_r($this->_db->errorInfo()));
    ...
    public function getCaisseById($id){
        $query = $this->_db->prepare(' SELECT * FROM t_caisse_iaaza WHERE id=:id')
        or die (print_r($this->_db->errorInfo()));
        $query->bindValue(':id', $id);
        $query->execute();      
    ...
    public function getTotalCaisseByType($type){
        $query = $this->_db->prepare("SELECT SUM(montant) as total FROM t_caisse_iaaza WHERE type=:type")
        or die (print_r($this->_db->errorInfo()));
    ...
    public function getCaisses(){
        $caisses = array();
        $query = $this->_db->query('SELECT * FROM t_caisse_iaaza ORDER BY type ASC, dateOperation DESC');
    ...
    public function getCaissesByType($type){
        $caisses = array();
        $query = $this->_db->prepare('SELECT * FROM t_caisse_iaaza WHERE type=:type ORDER BY dateOperation');
        $query->bindValue(':type', $type);
        $query->execute();
        while($data = $query->fetch(PDO::FETCH_ASSOC)){
            $caisses[] = new Caisse($data);
    ...
    public function getCaissesByDates($dateFrom, $dateTo){
        $caisses = array();
        $query = $this->_db->prepare('SELECT * FROM t_caisse_iaaza WHERE
    ...
    public function getCaissesByLimits($begin, $end){
        $caisses = array();
        $query = $this->_db->query('SELECT * FROM t_caisse_iaaza
    ...
    public function getLastId(){
        $query = $this->_db->query(' SELECT id AS last_id FROM t_caisse_iaaza
