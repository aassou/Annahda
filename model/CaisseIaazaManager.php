...
class CaisseIaazaManager{
    ...
    public function getCaisses(){
        $caisses = array();
        $query = $this->_db->query('SELECT * FROM t_caisse_iaaza ORDER BY dateOperation DESC');
    ...
    public function getCaissesByType($type){
        $caisses = array();
        $query = $this->_db->prepare('SELECT * FROM t_caisse_iaaza WHERE type=:type ORDER BY dateOperation DESC');
        $query->bindValue(':type', $type);
        $query->execute();
        while($data = $query->fetch(PDO::FETCH_ASSOC)){
            $caisses[] = new Caisse($data);
